...
namespace Email
{
    class EmailOutputChannel : ChannelBase, IOutputChannel
    {
        ...
        public EmailOutputChannel(EmailChannelFactory fact, EndpointAddress epa, Uri via, MessageEncoder enc) : base(fact) {
            encoder = enc;
            this.epa = epa;
            if (via.Scheme != "mailto")
            {
                throw new InvalidOperationException("invalid address scheme. Must be 'mailto'");
            }
            emailaddress = via.UserInfo + "@" + via.Host;
            this.fact = fact;
